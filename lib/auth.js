...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    enabled: true,
    autoSignIn: true,
    // Closes the privilege-escalation path where an unverified sign-up
    // could still get a session whose email is_staff()/can_see_money() (and
    // api/account.js) trust outright. All known-legitimate accounts were
    // backfilled to emailVerified = true directly in the live DB before
    // this flipped, so no existing sign-in is locked out — only new,
    // never-verified signups must complete a code/link first.
