 ...
 * sign-in, Google OAuth callback, and session cookie must resolve there, not
 * the *.vercel.app deployment URL. Google only redirects back to the exact
 * registered callback (https://pistonpoweredranch.com/api/auth/callback/
 * google), and Better Auth sets the session cookie on whatever host baseURL
 * points at, so baseURL has to be the custom domain or logins land on the
 * wrong host and the cookie never reaches the pages the user actually visits.
 *
