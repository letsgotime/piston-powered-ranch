...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    jwt({
      /* The one row already in neon_auth.jwks (187b8b45...) was minted by
         Neon's hosted auth on 26 Aug and its private half is encrypted with
         Neon's secret, not BETTER_AUTH_SECRET. This server can publish that
         key but can never sign with it: every /api/auth/token call threw
         "Failed to decrypt private key" and answered 500, so no signed in
         person ever got a database token, and every staff page read as
         "Not signed in". Hide that row from the plugin and it does what it
         does on a fresh install: mints its own pair on the first request and
         publishes the public half at /api/auth/jwks, which is the same table
         Neon's JWKS URL serves, so the Data API sees the new key too. */
      adapter: {
        getJwks: async (ctx) => {
          const rows = await ctx.context.adapter.findMany({ model: "jwks" });
          return rows.filter((k) => k.id !== "187b8b45-56f1-45df-88a8-43e0a7fe72eb");
        },
        /* The table was made by Neon's hosted auth with an older Better Auth
           schema: id, publicKey, privateKey, createdAt, expiresAt. The plugin
           in this version also writes alg and crv, and Postgres refused the
           insert, which took get-session down with it. Write the shape the
           table has; alg falls back to EdDSA and crv is inside the public
           JWK itself, so nothing downstream misses them. */
